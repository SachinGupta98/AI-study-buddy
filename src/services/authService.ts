...
// NOTE: This is a mock function for a demo.
...
export const forgotPassword = async (username: string): Promise<string> => {
    ...
    // A real app NEVER stores retrievable passwords.
    // In a real backend, you'd find the user by email and send a reset link.
    return `A password reset link would be sent to the email associated with '${username}'. As this is a demo, please create a new account if you have forgotten your password.`;
};
...
export const logout = () => {
    // BUG FIX: Only clear the session, not the entire localStorage.
    // This preserves the user database across sessions.
    ...
    (user.studyPlans || []).forEach(plan => {
        ...
        (plan.weekly_plans || []).forEach(week => { // week.week is 1-based
            (week.daily_tasks || []).forEach(day => {
                const allTasksInDayCompleted = day.tasks.length > 0 && day.tasks.every(t => t.completed);
