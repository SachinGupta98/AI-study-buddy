    ...
    if (error instanceof SyntaxError) {
        ...
        switch (context) {
            ...
            case 'getAICoachInsight':
                ...
                break;
            case 'generateSpeechFromText':
                userMessage = "Failed to generate audio for this message.";
                break;
        }
    }
...
const MAX_HISTORY_LENGTH = 20; // Keep the last 20 messages (10 turns)
...
export const getGeneralChatResponseStream = async (
    ...
    // PERF FIX: Replace complex summarization with simple, safe truncation.
    // Keep the first message (initial greeting) and the most recent messages.
    const historyForModel = history.length > MAX_HISTORY_LENGTH
        ? [history[0], ...history.slice(-MAX_HISTORY_LENGTH)]
        : history;
    ...
    const chat = ai.chats.create({
        model: 'gemini-2.5-pro',
        history: transformHistory(historyForModel),
        config: chatConfig,
    });
...
export const generateSpeechFromText = async (text: string): Promise<string> => {
    try {
        ...
        throw handleGeminiError(error, 'generateSpeechFromText');
    }
};
